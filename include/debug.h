...
*******************************************************************************/
#define mInitSTDIO() mInitSTDIOBaud(UART0_BAUD)
//uart_spped is defined as baud rate divided by 10
inline void	mInitSTDIOBaud(uint32_t uart_baud)
{
    uint32_t x;
    uint8_t x2;
    ...
    PCON |= SMOD;
    x = 10 * FREQ_SYS / uart_baud / 16;                                       //If you change the main frequency, be careful not to overflow the value of x
    x2 = x % 10;
    x /= 10;
    if ( x2 >= 5 ) x ++;                                                       //rounding
